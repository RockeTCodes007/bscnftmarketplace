...
function Mint() {
  ...
  useEffect(() => {
    ...
    async function loadContract() {
      tokenContract = new web3.eth.Contract(
        config.tokenContract.abi,
        config.tokenContract.address
      );
      playableTokenContract = new web3.eth.Contract(
        config.playableNFTContract.abi,
        config.playableNFTContract.address
      );
    }
    loadContract();
  }, []);
  ...
  useEffect(() => {
    if (videoNFTMetadata.uploaded) {
      toastInfo(
        "Video Upload success! Go ahead and click the mint button to mint!"
      );
    }
  }, [videoNFTMetadata]);
  ...
  async function mint(hash) {
    ...
    try {
      const contract =
        mintType === "img" ? tokenContract : playableTokenContract;
      const res = await contract.methods
        .safeMint(activeAcc, hash.toString())
        .send({
          from: activeAcc,
        });
  ...
  function getVideoId() {
    ...
    let uniqueId = videoNFTMetadata.url.replace(
      ...
      <Box className={styles.mintPageContainer}>
        <div>
          ...
          <Typography variant="subtitle1" style={{ color: "white" }}>
            All of your NFT files, including metadata and assets, are stored on
            the decentralized Pinata IPFS network. This means that your NFTs are
            safe from censorship and single points of failure.
          </Typography>
        ...
        <Select
          id="type"
          onChange={(e) => setmintType(e.value)}
          options={OPTIONS}
        />
        {mintType === "img" && (
          <div className={styles.ImgMintContainer}>
            <div>
              <DisplayNFT
                owner="test"
                type={mintType}
                file={imgBytes ? imgBytes : null}
              />
            </div>
        ...
        {mintType === "vid" && (
          <div className={styles.vidMintContainer}>
            <UploadWidget
              setNftDetails={setvideoNFTMetadata}
              nftDetails={videoNFTMetadata}
            />
            <Button
              onClick={proceedToMint}
              className={styles.mintBtn}
