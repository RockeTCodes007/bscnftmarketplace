...
function App() {
  ...
  useEffect(() => {
    ...
    if (!isConnectedToBSC()) {
      setTimeout(() => {
        toastInfo(
          "You are not connected to BSC, pleae switch to BSC to run full features of the app!"
        );
      }, 2000);
      setTimeout(() => {
        switchToBSC();
    ...
    <>
      <BrowserRouter>
        <WalletContext.Provider
          value={{ connect, activeAcc, checkIfConnected, disconnect }}
        >
          <Header />
          <Routes>
